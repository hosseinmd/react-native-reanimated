...
class AnimatedProps extends InternalAnimatedNode {
  ...
  private _animatedView: any;

  constructor(props: any, config: any, callback: any) {
    super({ type: 'props', props: config }, Object.values(props).filter(
      (n) => !(n instanceof AnimatedEvent)
    ) as any);
