...
export default class AnimatedNode<T extends Value = string | number | boolean> {
  /**
  ...
  private __lastLoopID = { '': -1 };
  private __memoizedValue = { '': null };

  protected __children = [];
  protected __nodeConfig: any;
  protected __inputNodes: AnimatedNode[];
  __nodeID: any;

  constructor(nodeConfig: object, inputNodes?: Adaptable<Value>[]) {
  ...
  protected __forceUpdateCache(newValue) {
    this.__memoizedValue[callID] = newValue;
    this.__markUpdated();
  }

  private __dangerouslyRescheduleEvaluate() {
  ...
  protected _connectAnimatedView(nativeViewTag) {
  ...
  protected _disconnectAnimatedView(nativeViewTag) {
