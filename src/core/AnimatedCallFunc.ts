import InternalAnimatedNode, { getCallID, setCallID } from './AnimatedNode';
import { adapt } from './AnimatedBlock';
import { val } from '../val';
import invariant from 'fbjs/lib/invariant';
import { AnimatedNode } from '../types';

class AnimatedCallFunc extends InternalAnimatedNode {
  _previousCallID;
  _what;
  _args;
  _params;
  constructor(what, args, params) {
    ...
    invariant(
      args.every((el) => el instanceof InternalAnimatedNode),
      `Reanimated: every AnimatedCallFunc 'args' argument should be of type AnimatedNode but got ${args}`
    );
    invariant(
      params.every((el) => el instanceof InternalAnimatedNode),
...
export function createAnimatedCallFunc(proc, args, params) {
  return (new AnimatedCallFunc(
    ...
    args.map((p) => adapt(p)),
    params
  ) as unknown) as AnimatedNode<number>;
}
