...
function sanitizeArgMapping(argMapping: { nativeEvent: any }[]) {
  // Find animated values in `argMapping` and create an array representing their
  // key path inside the `nativeEvent` object. Ex.: ['contentOffset', 'x'].
  const eventMappings = [];
  const alwaysNodes = [];

  const getNode = (node: InternalAnimatedValue<any>) => {
    if (
      Platform.OS === 'web' ||
      Platform.OS === 'windows' ||
      Platform.OS === 'macos'
  ...
  if (typeof ev === 'object') {
    ...
    const proxyHandler = {
      ...
      set: function(target: { [x: string]: any }, prop: string, value: any) {
        if (prop === '__val') {
          target[prop] = value;
          return true;
        }
...
export default class AnimatedEvent extends AnimatedNode {
  ...
  attachEvent(viewRef: any, eventName: string) {
  ...
  detachEvent(viewRef: any, eventName: string) {
    for (let i = 0; i < this._alwaysNodes.length; i++) {
...
export function createAnimatedEvent(argMapping: any, config: any) {
